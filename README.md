...
- **自动登录**：扫码登录，登录状态持久化
- **同步抓取**：题目截图 + 答案图片下载 + OCR 文字识别
- **持久化浏览器**：浏览器一次启动、多次抓取，减少资源占用、降低反爬风险
- **并行下载**：多张答案图片同时下载，OCR 并行识别
...
```
zujuan-output/
├── q_1234567890_0_question.png   # 题目截图（仅题目内容区域）
...
[
  {
    ...
    "questionText": "若函数 f(x) 在 x=1 处可导...",
    "answerText": "B",
...
> 注意：`answerText` 由 OCR 从答案图片识别而来，准确度取决于图片质量和 Tesseract 识别效果。
...
当前使用 Tesseract.js 进行本地 OCR，准确度受以下因素影响：
